...
static const struct file_operations weasel_list_fops = {
	.open = weasel_list_open,
	.read = seq_read,
	.llseek = seq_lseek,
...
static int weasel_list_open(struct inode *inode, struct file *file)
{	
	/*permet d'envoyer ligne par ligne notre fichier*/
	return simple_open(file, weasel_list_show, inode->i_private);
}

static int weasel_list_show(struct seq_file*m, void *v)
{
	struct dentry *cur;
